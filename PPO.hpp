...
class PPO {
    ...
    PPO( const size_t state_size, const size_t action_count, const Hyperparameters& hyperparams );
...
// CONSTRUCTORS
PPO::PPO() { }
PPO::PPO( const size_t state_size, const size_t action_count, const Hyperparameters& hyperparams ) : hyperparams(hyperparams), rnd_gen(std::mt19937(std::random_device{}())){
    // Set LINEAR for mean outputs and EXP for log stddev outputs (to get normal stddev)
    std::vector<NeuralNetwork::Activation> o_activations(action_count*2);
    for(size_t i = 0; i < action_count; ++i)
        o_activations[i] = NeuralNetwork::LINEAR;
    for(size_t i = action_count; i < action_count*2; ++i)
        o_activations[i] = NeuralNetwork::POW_1P1;
    const NeuralNetwork::OutputActivations outputs_activation = NeuralNetwork::ACTIVATIONS(o_activations);

    // Initialize networks
    value_network = NeuralNetwork( state_size, 1, hyperparams.value_hidden_layer_size, hyperparams.value_hidden_layer_count, NeuralNetwork::RELU, NeuralNetwork::ACTIVATIONS(NeuralNetwork::LINEAR, 1) );
    policy_network = NeuralNetwork( state_size, action_count*2, hyperparams.policy_hidden_layer_size, hyperparams.policy_hidden_layer_count, NeuralNetwork::RELU, outputs_activation);
    ...
    for(int k = 0; k < hyperparams.value_epochs; ++k){
        ...
        while(l < timestate_count){
            ...
            if( hyperparams.value_regularization_strength > 0)
                value_network.addRegularization(total_weight_grad, total_bias_grad, hyperparams.value_regularization_strength); 
    ...
    for(int k = 0; k < hyperparams.policy_epochs; ++k){
        ...
        while(l < timestate_count){
            ...
            if(hyperparams.policy_regularization_strength > 0)
                policy_network.addRegularization(total_weight_grad, total_bias_grad, -hyperparams.policy_regularization_strength); 
            // Add momentum and root mean square prop
